...
library(gwascat)
ebicat37 <- makeCurrentGwascat(genome = 'GRCh37')
```
...
Also we need to add `chr` to the `seqnames` to make this object compatible with `hg`

```{r}
seqlevels(ebicat37) <- str_c('chr',seqlevels(ebicat37))
```
